...
LEAGUE_ID = "ISR1"
OUTPUT_FILE = "israel_league_2010_2025.csv"
...
def scrape_transfermarkt():
    with sync_playwright() as p:
        ...
        for season in SEASONS:
            ...
            for matchday in range(1, 37):
                url = f"https://www.transfermarkt.com/ligat-haal/spieltag/wettbewerb/{LEAGUE_ID}/saison_id/{season}/spieltag/{matchday}"
                
                try:
                    print(f"Scraping Season {season} - Matchday {matchday}...")
                    page.goto(url, timeout=60000)
                    
                    try:
                        page.wait_for_selector("div.box", state="visible", timeout=10000)
                    except:
                        print(f"  Skipping MD{matchday} (no data or timeout)")
                        continue
                    ...
                    if day_data:
                        ...
                        print(f"  Saved {len(day_data)} matches.")

                    time.sleep(random.uniform(2.0, 4.0))

                except Exception as e:
                    print(f"  Error on S{season}-MD{matchday}: {e}")
                    time.sleep(5)

        browser.close()
        print("Scraping Complete!", OUTPUT_FILE)
